...
from .tools.tools import filtra_url, aplica_regla, sentimiento, nube, entidades, enviar_telegram, \
    enviar_telegram_estadistica, telegram_norep
import os
import datetime
...
class Medios(models.Model):
    ...
    importancia = fields.Selection(
        [('baja', 'Baja'), ('media', 'Media'), ('alta', 'Alta'), ('prueba', 'Prueba'), ('nuevo', 'Nuevo'),
         ('cat1', 'Categoría #1'), ('cat2', 'Categoría #2'), ('cat3', 'Categoría #3')])
    ...
    def scrap_noticias(self, importancia="todos", tipo="", pagina=""):
        ...
        all_records2 = self.env['wsf_noticias_medios'].search(filtro_importancia, order='medio asc')
        ...
        for rec in all_records:
            ...
            if tipo == "prueba":
                if rec.pagina_web != pagina and rec.pagina_rss != pagina:
                    ...
                    self.prueba = "Comenzando a tomar información del portal a las: " + datetime.datetime.now().strftime(
                        '%Y/%m/%d %H:%M:%S') + "\n\n"
                    self.reglas = "Comenzando a visualizar las reglas: " + datetime.datetime.now().strftime(
                        '%Y/%m/%d %H:%M:%S') + "\n\n"
            else:
                            ...
                            for entrada in v.entries:
                                ...
                                try:
                                    ...
                                    if encontrado and tipo != "prueba":
                                        ...
                                        url_medio2 = valor['rss'].replace("https", "http")
